...
class ClientRepository extends UserRepository
{
    ...
    public function assignClient(int $id_user): bool
    {
        ...
        if ($this->isClient($id_user)) {
            return false; // Déjà client, on ne fait rien
        }

        // Ajoute dans la table Client
        $stmt = $this->database->prepare("INSERT INTO client (id_user) VALUES (:id_user)");
